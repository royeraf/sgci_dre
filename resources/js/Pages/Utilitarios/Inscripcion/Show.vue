        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="relative overflow-hidden rounded-3xl sm:rounded-[2rem] text-white mb-6 sm:mb-8 shadow-2xl shadow-black/20">
                ...
                <div class="relative z-10 px-6 py-8 sm:px-10 sm:py-10 md:px-14 md:py-12">
                    ...
                    <div class="flex flex-wrap items-center gap-x-2.5 gap-y-2 text-xs sm:text-sm text-white/70 mt-6 max-w-xl">
                        <span class="flex items-center gap-1.5 whitespace-nowrap">
                            ...
                            <span v-if="evento.hora_inicio">· {{ formatHora(evento.hora_inicio) }}<span v-if="evento.hora_fin"> - {{ formatHora(evento.hora_fin) }}</span></span>
                        </span>
                        <span class="w-1 h-1 rounded-full bg-white/20"></span>
                        <span class="flex items-center gap-1.5 whitespace-nowrap">
                            <component :is="modalidadIcon(evento.modalidad)" class="w-3.5 h-3.5 text-[var(--accent-light)] flex-shrink-0" />
                            {{ modalidadLabel(evento.modalidad) }}<span v-if="evento.lugar || evento.enlace_virtual"> · {{ evento.lugar || evento.enlace_virtual }}</span>
                        </span>
                        <template v-if="evento.expositores.length">
                            <span class="w-1 h-1 rounded-full bg-white/20"></span>
                    ...
                    <div v-if="evento.cupo_maximo || evento.horas_educativas" class="flex flex-wrap items-center gap-2 mt-4">
                        <span v-if="evento.cupo_maximo"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-white/[0.07] border border-white/10 text-white/80">
                            <Users class="w-3.5 h-3.5 text-[var(--accent-light)]" />
                            {{ Math.max(evento.cupo_maximo - evento.inscritos_count, 0) }} cupo(s) disponible(s) de {{ evento.cupo_maximo }}
                        </span>
                        <span v-if="evento.horas_educativas"
                            class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-white/[0.07] border border-white/10 text-white/80">
                            <Clock class="w-3.5 h-3.5 text-[var(--accent-light)]" />
                            {{ evento.horas_educativas }} horas educativas
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div v-else class="bg-white rounded-3xl shadow-xl p-5 sm:p-8 border border-slate-100">
                ...
                <form @submit.prevent="onSubmit" class="space-y-4">
                                ...
                                <div class="flex gap-2">
                                    <div class="flex-1 relative">
                                        <input v-model="numeroDocumento" v-bind="numeroDocumentoProps" type="text"
                                            ...
                                            class="w-full px-4 py-3 border-2 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-4 focus:ring-[var(--accent-20)] focus:border-[var(--accent)] bg-white transition-all duration-200 outline-none font-mono tracking-wider"
                                    ...
                                    <button v-if="tipoDocumento === 'DNI'" type="button" @click="buscarPorDni"
                                        ...
                                        class="px-4 py-3 bg-gradient-to-r from-[var(--accent)] to-[var(--accent-dark)] text-white rounded-xl hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1.5">Género</label>
                            <select v-model="genero" v-bind="generoProps"
                                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl text-slate-900 focus:ring-4 focus:ring-[var(--accent-20)] focus:border-[var(--accent)] bg-white transition-all duration-200 outline-none cursor-pointer">
...
import { Calendar, MapPin, Video, Users, Mic, UserPlus, Loader2, AlertCircle, CheckCircle2, Search, Check, X, ChevronDown, Clock } from 'lucide-vue-next';
...
const inscripcionSchema = toTypedSchema(
    yup.object({
        ...
        genero: yup.string().transform((v) => v || null).nullable(),
