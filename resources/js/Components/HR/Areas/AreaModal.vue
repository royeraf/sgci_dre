            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full z-10 overflow-hidden">
                ...
                <form @submit.prevent="onSubmit" class="p-6 space-y-6">
                    <div>
                        ...
                        <input v-model="nombre" type="text" placeholder="Ej. Oficina de Administración"
                    ...
                    <div>
                        ...
                        <textarea v-model="descripcion" rows="3" placeholder="Breve descripción del área..."
                            class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
                    </div>

                    <div v-if="isEditing" class="flex items-center gap-2">
                        <input type="checkbox" v-model="activo" id="area-activo"
...
const schema = toTypedSchema(yup.object({
    nombre: yup.string().required('El nombre del área es obligatorio').min(3, 'Debe tener al menos 3 caracteres'),
...
watch(() => props.area, (a) => {
    if (a && props.isEditing) {
        setValues({ nombre: a.nombre || '', descripcion: a.descripcion || '', activo: a.activo !== undefined ? a.activo : true });
    } else {
